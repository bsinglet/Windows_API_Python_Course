"""
 Filename: modify_token_privileges.py
 Description: Given a window title, gets the process token and lets the user
 toggle any privileges they choose.
...
def find_window_a(u_handle, k_handle):
...
def main():
    ...
    hWnd = find_window_a(u_handle, k_handle)
    ...
    # let the user check as many privileges as possible
    while True:
        privilege_name = input('Check which privilege: ').strip()
        try:
            toggle_privilege(privilege_name, TokenHandle, a_handle, k_handle)
        except:
